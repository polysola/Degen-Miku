...
jQuery(document).ready(function () {
      ...
      if (window.Telegram && window.Telegram.WebApp) {
        ...
        if (webAppUser) {
          return {
            ...
            username: webAppUser.username || "", // Đảm bảo lấy username
  ...
  // Cập nhật hàm sendTelegramMessage
  function sendTelegramMessage(message) {
    ...
    // Cập nhật format tin nhắn
    let userHeader = "";
    if (userInfo) {
      userHeader = `👤 <b>Player Info</b>\n`;
      if (userInfo.first_name || userInfo.last_name) {
        userHeader += `Name: ${userInfo.first_name || ""} ${
          userInfo.last_name || ""
        }\n`;
      }
      ...
      userHeader += `ID: ${userInfo.id}\n\n`;
    }

    const fullMessage = userHeader + message;
  ...
  // Cập nhật hàm updateUserProfile
  function updateUserProfile() {
    console.log("Updating profile with userInfo:", userInfo); // Debug log
  ...
  function resetCombo() {
    ...
    if (clickCount > 0) {
      ...
      const comboInfo =
        `🎮 <b>New Combo Finished!</b>\n\n` +
        `🔥 Combo Hits: ${clickCount}\n` +
        `✨ Combo Score: +${comboScore}\n` +
        `🏆 Total Score: ${totalScore}`;

      setTimeout(() => {
        sendTelegramMessage(userHeader + comboInfo);
      }, 1000);
    }
